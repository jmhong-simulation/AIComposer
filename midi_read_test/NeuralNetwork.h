...
public:
    ...
    Array1D<MatrixMN<D> > weights_, weights_backup_;                  // weights_[0] is between layer 0 and layer 1. 
    Array1D<MatrixMN<D> > delta_weights_, delta_weights_backup_;            // for momentum
    ...
    void initialize(const VectorND<unsigned>& num_layer_acts_, const int& _num_hidden_layers, const int& num_records)
    {
        ...
        // backup weights
        weights_backup_.initialize(num_all_layers_ - 1);                  // Note -1. Weight matrices are between layers.
        for (int l = 0; l < weights_backup_.num_elements_; l++)
        {
            // row x column = (dimension of next layer  - 1 for bias) x  (dimension of prev layer - this includes bias)
            weights_backup_[l].initialize(layer_neuron_act_[l + 1].num_dimension_ - 1, layer_neuron_act_[l].num_dimension_);// -1 is for bias. y = W [x b]^T. Don't subtract 1 if you want [y b]^T = W [x b]^T.
        ...
        delta_weights_backup_.initialize(num_all_layers_ - 1);
        for (int l = 0; l < delta_weights_backup_.num_elements_; l++)
        {
            // row x column = (dimension of next layer  - 1 for bias) x  (dimension of prev layer - this includes bias)
            delta_weights_backup_[l].initialize(layer_neuron_act_[l + 1].num_dimension_ - 1, layer_neuron_act_[l].num_dimension_);// +1 is for bias
        }
    ...
    void backupWeights()
    {
        for (int l = 0; l < weights_.num_elements_; l++)
        {
            weights_backup_[l] = weights_[l];

            delta_weights_backup_[l] = delta_weights_[l];
        }
    }
    ...
    void restoreWeights()
    {
        for (int l = 0; l < weights_.num_elements_; l++)
        {
            weights_[l] = weights_backup_[l];

            delta_weights_[l] = delta_weights_backup_[l];
        }
    }
    ...
    int getIXProbOutput()
    {
        return getIXProbOutput(0, num_output_ - 1);
    }

    int getIXProbOutput(const int& d_min, const int& d_max)
    {
        const VectorND<D>& output_layer_act(layer_neuron_act_[layer_neuron_act_.num_elements_ - 1]);
